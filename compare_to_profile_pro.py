...
def safe_component_number(value, fallback=None):
    try:
        if value is None:
            return fallback
        number = float(value)
...
def fill_visible_score_components(item: Dict[str, Any]) -> Dict[str, Any]:
    """Ensure frontend cards never show blank component boxes.

    Artist Match V2 is mostly track-library/fingerprint based. The old UI still
    expects Frequency, Arrangement, and Core component fields. When V2 cannot
    calculate one of those directly, this fills it with the best available
    stable fallback so the UI stays complete without changing the ranking.
    """
    components = item.get("score_components") or {}
    if not isinstance(components, dict):
        components = {}

    match_score = safe_component_number(item.get("match_score"), 0)
    raw_score = safe_component_number(item.get("raw_match_score"), match_score)
    fingerprint = safe_component_number(
        components.get("fingerprint"),
        safe_component_number(components.get("prototype"), raw_score),
    )

    frequency = safe_component_number(
        components.get("frequency"),
        safe_component_number(components.get("style"), fingerprint),
    )

    arrangement = safe_component_number(
        components.get("arrangement"),
        safe_component_number(components.get("prototype"), match_score),
    )

    core = safe_component_number(
        components.get("core"),
        safe_component_number(components.get("overall_metrics"), match_score),
    )
...
def compare_audio_to_profiles(
    ...
    if ranked:
        ...
        for index, item in enumerate(ranked):
            ...
            fill_visible_score_components(item)
    ...
    ranked = [fill_visible_score_components(item) for item in ranked]
